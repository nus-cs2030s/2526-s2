...
For instance, in C#, every instantiation of a generic type causes new code to be generated for that instantiated type.  Instantiating `Pair<S,T>` into `Pair<String,Integer>` causes a new type to be generated during runtime.  In C++ and in Rust, instantiating `Pair<String,Integer>` causes new code to be generated during compile-time.  This approach is sometimes called _code specialization_, in contrast to Java's _code sharing_ approach.
